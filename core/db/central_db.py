...
class CentralDB:
    ...
    def __init__(self, db_path: str = "data/central.db"):
    ...
    def remove_location(self, user_id: int, location_id: str) -> bool:
        with self._get_connection() as conn:
            ...
            return True
    def get_location_by_id(self, user_id: int, location_id: str) -> dict:
        """
        Возвращает локацию пользователя по её ID.
        """
        with self._get_connection() as conn:
            row = conn.execute("""
                SELECT location_id, display_name, lat, lon, is_default 
                FROM user_locations 
            ...
            raise ValueError(f"Location {location_id} not found for user {user_id}")